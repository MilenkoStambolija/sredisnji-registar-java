...
specificTerms = new HashMap<>();
    ...
    private String refineDescription(String opisFormat, String vrstaVezePrikaz, Osoba targetOsoba, Osoba povezanaOsoba) {
        ...
        switch (vrstaVezePrikaz.split(" / ")[0]) {
            ...
            case "brat":
            case "sestra":
                refinedOpis = String.format("%s i %s su %s", targetOsoba.getPunoImeOrOib(), povezanaOsoba.getPunoImeOrOib(), vrstaVezePrikaz);
                break;
            case "polubrat":
            case "polusestra":
                refinedOpis = String.format("%s i %s su %s", targetOsoba.getPunoImeOrOib(), povezanaOsoba.getPunoImeOrOib(), vrstaVezePrikaz);
    ...
    private void checkGreatGrandRelationships(Osoba targetOsoba, List<VezaDTO> veze, Set<List<String>> processedRelations) {
        ...
        // Find great-grandparents (parents of grandparents)
                ...
                .forEach(rowMap -> {
                    ...
                    if (prabaka != null) {
                        addRelationshipToResults(veze, targetOsoba, prabaka, "praunuk_praunuka_pradjed_prabaka",
                            "{target} je %s osobe {povezana}".formatted(getRelationshipDisplayTerm("praunuk_praunuka_pradjed_prabaka", targetOsoba.getSpol(), prabaka.getSpol(), null).split(" / ")[0]),
                    ...
                    if (pradjed != null) {
                        addRelationshipToResults(veze, targetOsoba, pradjed, "praunuk_praunuka_pradjed_prabaka",
                            "{target} je %s osobe {povezana}".formatted(getRelationshipDisplayTerm("praunuk_praunuka_pradjed_prabaka", targetOsoba.getSpol(), pradjed.getSpol(), null).split(" / ")[0]),
                            "Izvedena", "Uspravna", "3", processedRelations, null, false);
                    }
                });
        }

        // Find great-grandchildren (children of grandchildren)
        ...
        if (targetParentsRowMap != null) {
            ...
            csvDataLoader.maticaRodenihData.stream()
                ...
                .forEach(entryMap -> {
                    ...
                    if (!opis.isEmpty()) {
                        String vrstaVezePrikaz = getRelationshipDisplayTerm(vrstaVezeKljuc, targetOsoba.getSpol(), sibling.getSpol(), null);
